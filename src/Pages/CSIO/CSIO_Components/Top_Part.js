...
//!isShown
  if (!isShown){
    return (
      <div className="">
        ...
        <div className="bg-orange-200b relative z-0 flex justify-center content-around h-screen flex-wrap">
            <div className="flex font-bold items-center text-center py-6 text-6xl" onMouseEnter={() => setIsShown(true)} onMouseLeave={() => setIsShown(false)}>We help students have their voices heard.</div>
	      </div>
      </div>
    );
}
    ...
    <img className = "bg-black overflow-hidden absolute w-full opacity-50 top-0 z-0" width="100%" src={officers}/>
